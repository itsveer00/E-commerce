import React from "react";
import "./specialoffer.css";

const Specialoffer = () => {
  return (
    <>
      <section className="annoucement">
        <div className="container banner-images">
          <div className="banner-img1 banner">
            <img src="./assets/banner/banner-11.png" alt="banner-image" />
          </div>
          <div className="banner-img2 banner">
            <img src="./assets/banner/banner-22.png" alt="banner-image" />
          </div>
          <div className="banner-img3 banner">
            <img src="./assets/banner/banner-33.png" alt="banner-image" />
          </div>
        </div>
      </section>
    </>
  );
};

export default Specialoffer;
